...
| cancelled | cancelled |
| error | failed |
| 无 active run 但有可恢复 checkpoint | recovery_pending |
| 无 active run 且不可恢复 | orphaned_failed |

`degraded` 不是 Run 状态，通过 `completion_scope + warnings + artifact completeness` 表达。最大投影延迟：运行中 5 秒，终态 2 秒；超过阈值告警并进入 reconciliation。
