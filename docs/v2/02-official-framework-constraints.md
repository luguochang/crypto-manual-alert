...
| 进程内事件投影 | `streamEvents(..., { version: "v3" })` typed projections | 手动解析 stream-mode tuple 作为产品协议 |
...
1. **Agent/Graph 进程内**
   - 使用 `streamEvents(input, { version: "v3" })`。
   - 优先消费 `messages`、`toolCalls`、`values`、`output`、`subagents`、`subgraphs`、`extensions`。
   - 只有调试或官方 projection 尚未覆盖时才读取 raw protocol event。
2. **Agent Server**
...
固定 channel 为 `values`、`updates`、`messages`、`tools`、`lifecycle`、`input`、`tasks`、`custom`。业务 extension 必须命名为 `custom:<name>`，有 schema version、owner、retention 和 UI consumer。禁止创建与固定 channel 等价的 `custom:message`、`custom:tool`、`custom:interrupt`。
...
`ShellToolMiddleware`、`FilesystemFileSearchMiddleware`、`FilesystemMiddleware` 和生产代码执行默认禁止。`SubagentMiddleware` 只在任务确实需要委派时使用，并设置深度、模型、Tool、调用和权限预算。
...
- 生产必须启用 custom route auth，并保证 auth-first middleware order。
