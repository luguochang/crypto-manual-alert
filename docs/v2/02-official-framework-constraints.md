...
| PII/密钥脱敏 | `PIIMiddleware(apply_to_output=True, apply_to_tool_results=True)` + 自定义 secret detector + Trace masking | 页面层最后再字符串替换 |
...
| Python 进程内流 | `agent.astream_events(..., version="v3")`、`graph.astream(..., stream_mode=...)`、stream writer | 手动把 raw event/chunk 定义成产品协议 |
| Server command/event | Protocol v2 `/commands` + `/stream/events` | 自定义 POST+SSE envelope 和 sequence 规则 |
| Subagent 展示 | `stream.subagents` + scoped selector hooks | subagent polling、手工 namespace 状态树 |
| 自定义业务事件 | Middleware/Graph stream transformer + `custom:<name>` | 通用 EventBus/StreamingManager |
...
| 运行中消息与队列 | Client submission queue + Agent Server worker queue + multitask strategy | 混淆内存队列与持久任务队列，或私建 Agent queue runtime |
| 编辑/重生成/分支 | Checkpoint fork / `forkFrom` / time travel | 复制整段对话并伪造新历史 |
| 后台任务 | Agent Server background runs | 页面保持连接才允许任务继续 |
| 计划任务 | Agent Server crons | 应用进程内无持久化 timer |
| HITL 批量响应 | `respond()` / `respondAll()` | 自建 pending action 状态机 |
| Artifact/Generative UI | typed custom extension + 受控组件注册表 | 执行模型生成的任意 HTML/JSX |
...
Interrupt 节点恢复时从节点函数开头重执行。`interrupt()` 前不得发生非幂等副作用；外部调用、随机数、时间读取和写入应拆成独立幂等节点或 `@task`。同一节点内 interrupt 顺序属于持久化契约，不能随意调整。
...
`create_deep_agent` 的默认栈包含 Filesystem、Subagent、Summarization、Patch Tool Calls 等能力。生产实现必须显式断言最终 Tool/Middleware 清单；仅省略参数不代表默认 Tool 已关闭。完全禁用 filesystem 时优先改用可完全定制的 `create_agent` Research Harness，否则必须配置 deny-all/read-only permission 并测试“未匹配规则不得允许”。

Research subgraph 默认 `checkpointer=None` 继承父图，实现 per-invocation persistence。`checkpointer=True` 的 per-thread 模式只有在明确需要跨调用子 Agent memory 时才能启用，并禁止同一 subgraph 并行调用。
...
- 使用官方 `submit()`、`stop()`、`disconnect()`、`respond()`、`respondAll()` 和 thread history 能力。
- 明确 `disconnect()` 只断开订阅，`stop()` 默认取消当前服务端 Run。
- 通过 `onThreadId` 持久化首次创建的 Thread ID；切换/恢复同一 `threadId` 时使用 SDK hydration/reattach。
- 区分 `isThreadLoading`、运行 `isLoading`、optimistic projection 和 reconnect 状态。
- 使用 `useMessages`、`useToolCalls`、`useValues` 对 subagent/subgraph 做懒订阅。
- 使用 `useExtension` 读取最新 typed custom projection，`useChannel` 读取有界事件日志，`useChannelEffect` 执行无渲染 side effect。
- 使用 `useSubmissionQueue`、`forkFrom`、`multitaskStrategy`、checkpoint history 和官方 media selectors 实现相应产品能力。
- Root `stream.interrupts` 只处理 root namespace；subagent/subgraph Interrupt 必须保留官方 snapshot 中的 interrupt ID 和 namespace。
...
- 用原生 `EventSource` 直连 Protocol v2 POST-only endpoint 并声称支持自动 resume。
- 所有 subagent 默认全量订阅消息和 Tool 事件；只有展开或实际渲染时才建立 scoped subscription。
- 把 `useSubmissionQueue` 当作刷新后仍存在的持久任务队列。
- 逐个 `respond()` 处理同一 checkpoint 的多个 Interrupt；必须使用一次 `respondAll()`。

### 3.7 三层事件所有权

1. **Python Agent/Graph 进程内**
   - Agent Harness 优先使用 `astream_events(..., version="v3")`；Graph state/task/subgraph 使用 `graph.astream(..., stream_mode=[...], subgraphs=True)`。
...
3. **React**
   - 每个 Thread 一个根 `useStream`。
   - 页面组件只使用根快照和 selector hook，不直接解析 wire frame。
   - 新 Thread 首次提交后通过 `onThreadId` 保存服务端 ID；rejoin 必须重新挂载同一 ID。
   - Replay buffer 是有界运行时能力，产品历史不能依赖无限 wire event replay。

官方 channel 以锁定 Agent Server OpenAPI、`@langchain/protocol` 和 SDK types 的交集/兼容策略为准，不在业务代码中复制静态枚举。CI 必须做 schema contract test，兼容发布包可能出现的 `checkpoints` 等差异。业务 extension 必须命名为 `custom:<name>`，有 schema version、owner、retention 和 UI consumer。禁止创建与官方 message/tool/interrupt 等价的私有 channel。

### 3.8 Agent Middleware 角色矩阵

| Middleware | Conversation/Planning Agent | Research | Decision | Eval |
| --- | --- | --- | --- | --- |
| `TodoListMiddleware` | 使用 | 使用 | 禁止 | 可选 |
| `SummarizationMiddleware` | 长 Thread 使用 | 长研究使用 | 通常禁止 | 可选 |
| `ContextEditingMiddleware` | 使用 | 使用 | 谨慎 | 可选 |
| `ModelCallLimitMiddleware` | 使用 | 使用 | 使用 | 使用 |
| `ToolCallLimitMiddleware` | 使用 | 使用 | 无 Tool | 按需 |
| `ModelRetryMiddleware` | 使用 | 使用 | 窄范围使用 | 使用 |
| `ToolRetryMiddleware` | 使用 | 使用 | 无 Tool | 按需 |
| `ModelFallbackMiddleware` | 可配置 | 可配置 | 默认禁止 | 可配置 |
| `PIIMiddleware` | 使用 | 使用 | 使用 | 使用 |
| `HumanInTheLoopMiddleware` | 高风险 Tool | 计划批准可选 | 禁止替代风险门禁 | 禁止 |
| `LLMToolSelectorMiddleware` | 权限过滤后可用 | 大 Tool 集可用 | 无 Tool | 可选 |
| `ProviderToolSearchMiddleware` | 按 Provider | 按 Provider | 禁止 | 可选 |
| `RubricMiddleware` | 禁止作为权威 | 可做质量检查 | 禁止决定 RiskVerdict | beta、限次使用 |
| `LLMToolEmulator` | 测试 | 测试 | 测试 | 测试 |

`ShellToolMiddleware`、代码执行和可用的 filesystem 权限禁止。Deep Agents 受保护的 Filesystem/Subagent middleware 若无法移除，必须通过 HarnessProfile 关闭 general-purpose subagent，并将 filesystem 权限设为 deny-all；否则改用 `create_agent` Research Harness。最终 Tool/subagent/permission 清单必须由启动测试断言。

所有面向用户流式输出的 Agent 必须显式配置 `PIIMiddleware` 对模型 output 和 tool results 生效，并增加 API key、Authorization、Cookie、Bark key、LangSmith/Langfuse key 等自定义 detector。默认参数不满足 wire-level 脱敏目标；必须通过 Protocol v2 frame、Thread snapshot、Trace 和日志泄漏 canary 测试。

自定义 Middleware 仅允许通过官方 `before_agent`、`before_model`、`after_model`、`after_agent`、`wrap_model_call`、`wrap_tool_call` 和 stream transformer 扩展。执行顺序必须在测试中固定：before hooks 正序，after hooks 逆序，wrap hooks 嵌套。不得依赖偶然的 Middleware 顺序。

顶层 `StateGraph`、持久化、通知、Cron 和业务集成不属于 AgentMiddleware。Graph 层使用 `RetryPolicy`、durability、`interrupt()`、Runtime Context、事务 Outbox 和幂等策略。禁止为了复用 Middleware 把确定性节点伪装成 Agent Tool。
...
- 使用 `ModelRetryMiddleware` 时必须显式关闭或限制模型/Provider SDK 的 `max_retries`；同一调用只允许一个 retry owner。
- Protocol v2 必须锁定 Agent Server/API、Python SDK、JS SDK 和 React SDK 兼容组，不能只锁前端包。
- `@langchain/protocol` 仍为 0.x，必须显式锁定；OpenAPI、protocol package 和 React types 不一致时先停工修 contract/ADR，不能在业务 wrapper 中猜测字段。
...
- 产品代码禁止直接 import 或打包 V1 `crypto_manual_alert.workflow`、`agent_swarm`、`orchestration`、`research_pipeline`、旧模型客户端、Tool Executor、telemetry runtime 和前端 fallback。
...
### 8.5 Agent Server Route/Auth Gate

- custom routes 只允许 `/app/*` 和 `/internal/*` 前缀。
- 生产必须保证 auth-first middleware order 完成身份认证；custom route 代码随后显式执行 ActorContext authorization，不能假设官方存在 custom-route resource handler。
- CI 禁止 custom route 覆盖 `/threads`、`/runs`、`/assistants`、`/store` 等系统路径。
- Run admission 在 `threads.create_run` 校验权限、entitlement、quota 和预算。
- Store namespace 由后端强制改写为 tenant/user/purpose，不信任客户端 namespace。
- 未使用的 Assistants/Store/UI/MCP/A2A 路由按部署配置关闭。
...
说明文件 front matter 必须保存唯一 slice ID、phase、owner、commit/PR 和 status。CI 校验代码变更至少关联一份新增说明；若未做真实运行，只能标记该 slice 未达到真实证据出口，不能把阶段记为完成。
...
- [ ] 是否使用 Protocol v2 和 React selector，而不是自己处理 sequence、replay、namespace 或 subagent polling？
- [ ] Custom channel 是否有 typed schema、版本、owner、retention 和明确消费者？
- [ ] Middleware 是否按 Agent 角色装配，并验证执行顺序和权限边界？
- [ ] 自定义 wrapper 是否有业务价值？
- [ ] Agent 是否越权决定风险或执行副作用？
- [ ] 多用户字段是否完整传播？
- [ ] workspace entitlement、usage 和 quota 是否在后端强制，而不是只在 UI 隐藏？
- [ ] 模型/Tool 输出是否结构化并持久化？
- [ ] 普通前端是否出现 raw JSON？
- [ ] 观测失败是否会阻断业务？
- [ ] 中文注释是否解释了关键不变量？
