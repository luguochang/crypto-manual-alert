...
Expected: FAIL on the first inherited/missing exact dependency or because runtime schemas and the root stream hook do not exist. `npm ci` failure caused by an intentionally inconsistent lockfile is not accepted as the only RED signal.
...
Expected: FAIL because `real-provider-desktop`/`failure-injection-desktop` projects and the complete persisted analysis flow do not exist. Skipping the tests due to missing credentials is not acceptable when the corresponding project was explicitly selected.

- [ ] **Step 3: Extend the V2 stack and add failure injection**

Retain `fixture-desktop`/`fixture-pixel-7` from Task 10 and add `real-provider-desktop`, `real-provider-pixel-7`, `failure-injection-desktop` and `failure-injection-pixel-7`, each with exact `testMatch`, profile env and isolated Compose project. Failure scenarios are controlled through the test-only scenario API and reset before each test; the route/profile fail startup outside `local/test`. Release proof uses both real-provider projects; fixture tests cannot satisfy real-provider gates.
...
- [ ] **Step 1: Write release-evidence, parity and SLO contract tests**
