...
第一阶段使用固定开发 Member 身份，并通过额外测试 fixture 覆盖第二用户、第二租户和最小权限 Operator。开发身份不得拥有 Owner/Admin 或跨租户权限，完整硬门禁见 `11-core-object-access-recovery-contract.md`。
...
- 运行中可以追加消息：客户端 submission queue 只做临时 UX，官方 multitask strategy 处理直接提交；需要跨刷新持久保证时写 Product `task_commands` 并由 dispatcher 创建官方 Run。Agent Server worker queue 不承担未派发用户输入。
...
- 用户无需回到原聊天页即可 approve/edit/respond/reject；`ignore/dismiss` 只用于非阻塞通知，不能遗留未解决的 Interrupt。
...
`Thread`、`Task`、`Run`、`Subagent` 和 `Checkpoint` 是必须稳定的内部/产品对象，但不都占一级导航。桌面 Work 采用三栏：左侧 Thread/Task，中间对话与运行时间线，右侧 Artifact/Evidence/Risk Inspector。个人用户只有一个 Workspace 时默认不显示 Workspace 切换器。

移动端使用显式分层导航，不缩放桌面三栏：

- `/work` 首屏显示当前 Thread、Task 状态、结论摘要和输入区。
- Thread/Task 列表使用可返回的全屏抽屉；返回后恢复滚动、输入草稿和焦点。
- Artifact/Evidence/Risk 使用独立详情层，顶部保留来源 Task 和返回路径。
- 后台 Task 状态在顶部紧凑状态栏和 Inbox badge 常驻，不遮挡输入。
- Inbox 深链接带 `return_to` 返回原 Thread/Task；跨设备无本地返回栈时回到对应 Task Detail。
- 软键盘、中文 IME、safe area、44px 触控目标、横竖屏、附件选择和离线重连进入 Playwright 真机尺寸测试。
...
4. 用户返回 Tasks 或原 Thread，SDK 使用 sequence replay 重新附着；若 replay window 已过期，先从持久业务 snapshot/timeline projection 重建，再接续 live stream。
5. UI 不重复消息和 Tool card，恢复到最新可证明阶段；只保存摘要的历史必须明确标记，不能伪装成完整 token 回放。
6. 完成、失败或等待人工时，Inbox 和配置的通知渠道收到事件。

### 6.4 人工处理 Interrupt

1. 高风险外部动作或计划变更触发官方 Interrupt。
2. Inbox 显示原因、建议动作、影响、证据、过期时间和 schema 对应控件。
3. 用户 approve、reject、edit 或 respond；多个并行中断可统一 `respondAll()`，subagent Interrupt 必须携带 interrupt ID、namespace、checkpoint 和 response version。
4. 需要同时修正 State 时使用原子 respond update。
5. 原 Thread 恢复，所有页面实时同步，不创建隐藏的新任务。
6. 多设备重复响应时第一个有效提交获胜，其他页面显示“该请求已在其他位置处理”并刷新状态。
...
| Connection Banner | hydrating、live、stream_error、background_unknown | `isThreadLoading/isLoading/error` + Product Task API；不假设 SDK 暴露 reconnect 状态 |
...
- 同一 Thread 的新消息使用客户端 submission queue 或显式 multitask strategy；需要跨刷新/设备持久保证的输入进入 Product `task_commands` admission/dispatch，不把 Agent Server worker queue 描述成 pending-message queue。
...
- PII、密钥、Cookie 和 Authorization 在模型、Tool result、Protocol frame、Thread snapshot、Trace 和日志出口前脱敏；PIIMiddleware 显式启用 output/tool-results，并配自定义 secret detector。
