...
V2 不在 V1 的自研工作流、Agent Swarm 和兼容层上继续修补，也不把最终产品限制成一张“手工预警”页面。本目录固定商业化 C 端 Agent 产品边界、官方框架职责、代码约束、交付门禁和实施记录方式；用户已于 2026-07-13 批准进入 V2 Final 实施。
...
16. [15-V2 实施状态总账](./15-v2-implementation-status.md)
   - 当前工作树的完成证据、明确缺口和下一条关键路径；不反向修改 normative 基线。
17. [16-M3 Aggregate HITL 证据](./16-m3-aggregate-hitl-evidence.md)
   - M3 aggregate pause、官方 root/nested fixture、队列收敛、真实 Playwright 与非生产边界。
18. [19-Windows + Docker Desktop 开发交接](./19-windows-docker-development.md)
...
| 文档 | 分类 | 作用 |
| --- | --- | --- |
| `01`、`02`、`03`、`06`、`08`、`11`、`12`、`13`、`14`、Accepted ADR | Approved normative | 进入需求注册表；冲突按 `13`/`14` 的优先级解释 |
| `09` 的 D01-D15 表格行 | Approved normative regions | 只有明确锚定的 D01-D15 决策进入需求注册表，其余评审导航内容为 Informative |
