...
| 1 Dependency/Agent Server Bootstrap | `partial` | 精确依赖锁、`langgraph.json`、canonical graph export、`probe_agent_server.sh` 已存在；Task 1 contract tests 已纳入通过的 backend 全量 suite。 | image digest lock、`versions.json`、实施说明、review/attestation 缺失。当前实际 Agent Server 仍是 `langgraph dev`，依赖为 `langgraph-cli[inmem]`；没有 durable Agent Server、`langgraph up` 或生产持久化证明。 |
...
| 8 Product APIs/Agent Integration | `partial` | 最小 Product API 已支持 create analysis、run list、get task；`GET /api/v2/tasks/{task_id}` 支持显式 `run_id` 读取历史 attempt。已有 official `langgraph-sdk` Runs client（create/list/join/cancel）和 `TaskView.agent_stream` 的 assistant/thread/run stream-binding metadata，以及 provision/worker contracts。失败投影会白名单保留 `provider`、`error_type`、`attempt`，同时丢弃 raw response、authorization、endpoint 和 correlation 等非公开字段。这里的证据仅是 official SDK Run client 与 stream binding metadata，**不是完整 stream protocol**。 | Runs detail/Inbox/Workspace/feedback/commands 完整 API、Protocol v2 command/event contract、官方 HITL routing、restart durability、`langgraph up` 集成拓扑及 protocol probe artifacts 缺失。当前独立 Product FastAPI + Agent Server 拓扑还偏离 Accepted ADR 0001。 |
...
| 14 Production Gates/Legacy Removal | `not_started` | 根目录存在本地 Dockerfile/Compose 草案；`.dockerignore` 与 `.gitignore` 递归排除嵌套 `.env*`、PEM/key/archive、secret/credential 目录、`node_modules`、coverage、依赖/build/test reports、`.langgraph_api` 与 `*.pckl` 等内容，并有 nested-path contract/`git check-ignore` 证据。`agent-server-readiness` 使用有界 `restart: "on-failure:12"`，worker 仍等待 authenticated readiness 完成；无身份 TCP probe 只承担 liveness。该证据仍只构成本地开发切片。 | runtime 仍为 `langgraph dev` + inmem，且 Compose dual-service 偏离 Accepted ADR 0001。production packaging、hosted HTTPS、hosted security/release gates、SLO/load/backup/restore/key rotation/upgrade/rollback、SBOM/signing、requirement evidence、独立 review attestation、V1 parity/removal 均不存在；递归 ignore 与有界本地 readiness 不能被解释为 release 或 production-ready 证明。 |
...
| 范围 | 结果 | 解释 |
|---|---|---|
| Backend `APP_ENVIRONMENT=test .venv/bin/pytest -q backend/tests` | `362 passed, 24 skipped, 1 warning` | 当前 hermetic/local backend suite 为 green；24 个 skip 仍按未证明处理，不能用来宣称外部 provider、durability 或生产门禁完成。唯一 warning 是 FastAPI/Starlette TestClient dependency deprecation。 |
| Root migration/structure/deployment suite | `1147 collected；full run exit 0` | 根目录 `tests` 全量执行退出为 0，覆盖 V1 retirement accounting、V2 route ownership、formal docs 状态和容器命令契约；这是本地静态/contract evidence。 |
...
| Frontend static/unit/build | lint（零 diagnostic）、typecheck、production build、unit 均为 green；`126 passed` | canonical `NEXTAUTH_URL` redirect/fail-closed coverage 已纳入 unit；证明当前 frontend/BFF/view-model 切片可生产构建，不等于 hosted behavior、生产 OIDC 或真实 provider 主链通过。 |
...
- 当前 Agent Server 命令是 `langgraph dev`，依赖锁包含 `langgraph-cli[inmem]`/`langgraph-runtime-inmem`。Product PostgreSQL 的存在不能替代 Agent Server checkpoint/store durability；当前没有 restart persistence、`langgraph up`、官方 durable deployment 或生产 HA 证明。
...
- `.dockerignore` 的 `**` 规则和 `.gitignore` 的递归匹配已覆盖嵌套环境变体、coverage、依赖/build/test reports、`.langgraph_api` 和 `*.pckl` 等本地残留；Compose frontend local proof 明确运行在 development 且 healthcheck 同时验证 Product health 与 `/work`。但 Compose 仍运行 dev + inmem Agent Server，其独立 Product FastAPI + Agent Server dual-service 结构与 Accepted ADR 0001 的“独立 FastAPI + Agent Server：不采用”冲突，必须通过实现收敛或新 ADR 正式处理。
...
5. 在继续生产化前解决 Accepted ADR 0001 与当前 dual-service Compose 的冲突；之后补齐 full correlation、middleware/call budgets、Inbox/Library/Settings、hosted security 与 Task 12-14 release gates。
