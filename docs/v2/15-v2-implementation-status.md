...
当前 16 个计划项（Task 0、0B、1-14）的裁决为：`done=0`、`partial=16`、`blocked=0`、`not_started=0`。**V2 仍不是 production ready。** G0.1 canonical audit、G0.2 本地 zero-mock 主流程和 G0.3 canonical framework convergence 已形成 fresh 本地证据；唯一生产 Graph、官方 `create_agent`/structured output/HITL/SDK/`@langchain/react` 边界已经锁定，孤儿手写 runtime 已删除。ADR 0011 现固定 Aegra `0.9.24` 自托管开源 Server，本地真实 Provider 主链、Redis worker kill/reaper/`since` replay 和 QA 双实例 HA 已分别形成证据。Task 13 依据 ADR 0010 锁定受限 `deepagents==0.6.12`，当前已推进到 canonical Graph、Product background lifecycle、Scheduled Monitor/Cron、本地 retention/export/deletion、独立 report Artifact、typed frontend 和本地断开重连 Playwright。Hosted OIDC/HTTPS、多主体浏览器状态、生产入口/rolling upgrade、免费/自托管 OpenTelemetry 后端、load/SLO、安全供应链和 release attestation 仍未证明；外部 Bark/Web Push/Email 发送已按用户要求排除，不是开放门禁。本地、fixture 或 skip 证据不能替代 hosted/生产门禁。

## 2. Task 0-14 状态矩阵

| Task | 状态 | 已存在的代码/测试证据 | 缺失项与裁决依据 |
|---|---|---|---|
| 0 Immutable Normative Baseline | `partial` | 新增 generation-one builder、27-source policy、D01-D15 稳定 region/requirement anchors 和仓库候选预检；候选 blob 分类/哈希、supersession 与 mixed marker 均 fail-closed，Aegra/免费观测/外部通知排除语义已在权威入口收敛。 | `docs/v2/normative-baseline.json`、三段独立有序 review/attestation 和正式 immutable candidate 均不存在；结构预检或 dangling commit 不能替代 Task 0 批准。 |
| 0B Requirement Registry | `partial` | baseline/registry/repository-policy 合计 23 项 synthetic/temporary-Git/candidate contract tests 已通过；工具严格要求显式映射、immutable candidate、owner 与 pre-RED receipt。 | 真实提取在 `01-v2-product-and-architecture.md:17` 因缺 requirement anchor 正确失败；其余 approved sources 也尚未完成逐条审阅锚定和 task/proof mapping。`normative-baseline.json`、`requirements-registry.yaml`、正式 note/review/receipt 均不存在。 |
