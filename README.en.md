...
>
> On 2026-07-29, one complete live local mainline passed with OKX public data,
> Tavily, an OpenAI-compatible model, Aegra, a LangGraph interrupt, human approval,
> checkpoint resume, Artifact commit, Runs, and Library. This is local real-provider
> evidence, not hosted-production proof. Hosted HTTPS, real OIDC, production
> HA/DR/SLO, and target-environment release gates remain open. External
...
- **Secret-safe observability** through OpenTelemetry and a free self-hosted backend;
  LangSmith/Langfuse remain optional adapters, alongside controlled diagnostic routes.

## Latest Walkthrough

These frames come from the live local Compose walkthrough on 2026-07-29, not a
fixture. The Agent used OKX public data, Tavily, and the configured OpenAI-compatible
`gpt-5.5`, paused for human approval, resumed from the LangGraph checkpoint, and
committed a no-trade report because the evidence conflicted. This proves the local
mainline on that host; it does not prove hosted production.

![Signal Desk live local result](docs/assets/screenshots/signal-desk-live-result.png)

<details>
<summary>Show the Pixel 7 result and artifact detail</summary>

![Signal Desk Pixel 7 result](docs/assets/screenshots/signal-desk-live-result-mobile.png)

![Signal Desk Pixel 7 artifact detail](docs/assets/screenshots/signal-desk-live-artifact-mobile.png)

</details>

Run the deterministic UI regression without credentials. It covers queued/running/
succeeded states, structured results, gates, responsive layout, accessibility, and
secret-safe rendering, but remains fixture-only evidence:
...
This is the only user-maintained dotenv file: `backend/.env`. Never commit it.
...
When Docker must reach Clash or another host proxy, use `host.docker.internal`, not
container-local `127.0.0.1`:
...
A fully ready local stack has healthy `langgraph-api`, `langgraph-api-readiness`,
`command-worker`, `frontend`, PostgreSQL, and Redis services. The readiness sidecar
uses the same optional `backend/.env` reference to validate the selected search
provider; values are never baked into an image.

## Business Architecture and User Flow
...
The UI is projected from durable facts and never treats a transient stream event as
success by itself:
...
- LangSmith/Langfuse may be optional observability adapters but cannot be an install
  or production-delivery prerequisite.
