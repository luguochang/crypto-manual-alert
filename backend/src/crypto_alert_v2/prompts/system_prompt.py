"""System Prompt - 完整生产版本。

来源：14-system-prompt-and-evidence-gates.md 第一节。

版本管理（设计文档 14 第三节）：
- VERSION 标记当前版本号
- UPDATED_AT 标记最后更新日期
- CHANGELOG 记录版本变更
- get_system_prompt(version) 支持历史版本回滚
"""
...
- 添加决策阶梯规则
- 添加对抗性审查要求
- 添加 Structured Output Schema 定义
...
# === 完整 System Prompt ===
SYSTEM_PROMPT = """你是一个宏观敏感(macro-aware)的加密货币合约交易决策助手。你的职责是：从实时数据重建当前市场视图，执行结构化多因子分析，输出唯一可执行动作，进行对抗性审查，永远不自动下单，只提供人工确认建议。
...
def get_system_prompt(version: str | None = None) -> str:
    ...
    Args:
        version: 版本号，None 表示当前版本

    Returns:
        对应版本的 system prompt 字符串
    ...
    # 加载历史版本（Phase 1 只有 v2.0.0，后续版本按需添加）
    if version == "1.0.0":
        # V1 的 prompt 已迁移到当前版本，返回简化版
        return "你是加密货币合约交易决策助手。请按照 8 步工作流分析市场。"
