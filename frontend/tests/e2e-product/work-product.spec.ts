...
test("renders the normal Product projection from queue through success", async ({ page }, testInfo) => {
  ...
  expect(requests.some((request) => request.pathname.endsWith("/api/v2/decision-requests"))).toBe(true);
...
async function installProductFixture(
  ...
  await page.route("**/api/product/**", async (route) => {
    ...
    if (
      request.method() === "POST"
      && url.pathname.endsWith("/api/v2/decision-requests")
    ) {
      ...
      await route.fulfill({
        ...
        body: JSON.stringify({
          ...
          route: {
            ...
            task_type: submission.intent === "deep_research"
              ? "deep_research"
              : "market_analysis",
            missing_slots: [],
    ...
    await route.fulfill({
      ...
      contentType: "application/json",
      body: JSON.stringify({
        detail: `Unexpected fixture route: ${request.method()} ${url.pathname}`,
      }),
    });
  });
